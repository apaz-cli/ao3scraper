...
if len(sys.argv) != 2:
    print("Error: SERVER_IP argument required", file=sys.stderr)
    sys.exit(1)
...
def start_modal_process():
    ...
    process = subprocess.Popen(
        ...
        env={**os.environ, "SERVER_IP": SERVER_IP},
        shell=True
    )
    return process
    ...
    with ThreadPoolExecutor(max_workers=min(count, 20)) as executor:
        ...
        for _ in range(count):
            if shutdown_flag.is_set():
                break
            future = executor.submit(start_modal_process)
